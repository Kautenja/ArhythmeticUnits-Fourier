...
\usepackage[moduleName={Spectre},version={2.0.1}]{ArhythmeticUnits}
...
functions. \textbf{Spectre} brings this concept to the virtual Eurorack
...
Spectre provides a single monophonic \textbf{Input} channel with an
exponential input gain control ranging from $-\infty\,dB$(complete silence)
to $12\,dB$ (approximately four times the nominal level.)

\subsection{Run}

The \textbf{Run} button governs the buffering of incoming audio into the
analysis window. When illuminated, Spectre continuously buffers and analyzes
new input samples. When dark, Spectre maintains the display of the current
analysis image without updating it with additional frames.
...
sample edges and reduces leakage. Spectre offers a selection of 15 commonly
...
The \textbf{Y-Scale} parameter determines how the frequency bins are
...
contrast, \textit{Logarithmic} mode arranges the frequency bins on an
exponential scale, which more closely aligns with human pitch perception.
