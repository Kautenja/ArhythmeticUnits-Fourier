...
struct SpectrumAnalyzerSIMD : rack::Module {
 ...
 private:
    /// DC-blocking filters for AC-coupled mode.
    Filter::DCBlocker<rack::simd::float_4> dc_blockers_simd;
...
struct SpectrumAnalyzerDisplay : rack::TransparentWidget {
 ...
 public:
    ...
    ///
    explicit SpectrumAnalyzerDisplay(SpectrumAnalyzerSIMD* module_) :
...
struct SpectrumAnalyzerWidget : ThemedWidget<BASENAME> {
    ...
    ///
    explicit SpectrumAnalyzerWidget(SpectrumAnalyzerSIMD* module = nullptr) : ThemedWidget<BASENAME>() {
        setModule(module);
        // Screws
