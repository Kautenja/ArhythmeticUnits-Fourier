...
#ifndef DSP_MATH_CLIPPING_HPP
#define DSP_MATH_CLIPPING_HPP
...
/// @brief functions for clipping numbers to stable ranges.
namespace Clipping {
...
inline T leaky_hard(const T& x, const T& m = 0.2) {
    static_assert(std::is_floating_point<T>::value, "x must be floating point type");
    static constexpr T ONE = 1;
    return abs(x) > ONE ? sgn(x) * (ONE - m) + m * x : x;
}

/// @brief Clip the input value using square function.
///
/// @tparam T the type of data to compute
/// @param x the input value \f$x \in [<-1, >1]\f$
...
inline T arctangent(const T& x) {
    static_assert(std::is_floating_point<T>::value, "x must be floating point type");
    static constexpr T TWO = 2;
    return (TWO / pi<T>()) * atan((pi<T>() / TWO) * x);
}
...
inline T square_root(const T& x) {
    static_assert(std::is_floating_point<T>::value, "x must be floating point type");
    static constexpr T ONE = 1;
    return x / (sqrt(ONE + x * x));
}

/// @brief Clip the input value using a wrapping function.
///
/// @tparam T the type of data to compute
/// @param x the input value \f$x \in [<-1, >1]\f$
/// @param k the scale factor that controls the midpoint of the wrapping function
/// @returns the soft-clipped value \f$y \in [-1, 1]\f$
/// @details
/// A `k` value of 2 wraps entirely around the dynamic range; a `k` value of
/// 1 wraps around the midpoint of the dynamic range.
///
template<typename T>
inline T wrap(const T& x, const T& k = 2.0) {
    static_assert(std::is_floating_point<T>::value, "x, k must be floating point type");
    return x - k * std::trunc(x);
}

/// @brief Clip the input value using the variable hardness clipper.
///
/// @tparam T the type of data to compute
/// @param x the input value \f$x \in [<-1, >1]\f$
/// @param k the scale factor that controls the shape
/// @returns the clipped value \f$y \in [-1, 1]\f$
/// @details
/// This variable hardness soft-clipper function was originally introduced by
/// Laurent de Soras (laurent@ohmforce.com).
///
// template<typename T>
// inline T variable_hard(const T& x, const T& k = 10.f) {
//     static_assert(std::is_floating_point<T>::value, "x, k must be floating point type");
//     return sgn(x) * pow(atan(pow(abs(x), k)), (1.f / k));
// }
