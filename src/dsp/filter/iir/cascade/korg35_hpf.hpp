// A Korg35 high-pass filter (i.e., from the MS-10 and MS-20).
...
#ifndef DSP_FILTER_IIR_CASCADE_KORG35_HPF_HPP
#define DSP_FILTER_IIR_CASCADE_KORG35_HPF_HPP
...
/// @brief A Korg35 high-pass filter (i.e., from the MS-10 and MS-20).
...
struct Korg35HPF {
    ...
    inline T process(const T& value) {
        // calculate the result of the input high-pass filter
        const T x = Bilinear::direct_form_2_transpose(b_highpass, a_highpass, h[0], value);
        // calculate the result of the feedback filter cascade
        const T f1 = Bilinear::direct_form_2_transpose(b_highpass, a_highpass, h[1], y);
        const T f2 = Bilinear::direct_form_2_transpose(b_lowpass, a_lowpass, h[2], f1);
        y = Q * (x + f2);
        return y;
    }
};
...
#endif  // DSP_FILTER_IIR_CASCADE_KORG35_HPF_HPP
