// An Attack-Hold-Decay-Sustain-Release (AHDSR) envelope generator.
...
/// @brief An Attack-Hold-Decay-Sustain-Release (AHDSR) envelope generator.
struct AHDSR {
 public:
    /// the different stages in the envelope.
    enum class Stage { Stopped = 0, Attack, Hold, Decay, Sustain, Release, NumStages };

    /// the events that output from the envelope generator at the end of stages
    enum class Event { None = 0, EndOfAttack, EndOfHold, EndOfDecay, EndOfSustain, EndOfRelease };
 ...
 public:
    ...
    /// stopped. the gate extends through the attack & hold phases allowing it
    /// to act as a gate signal for an AHD LFO.
    inline bool isEOC() const {
        return static_cast<int>(stage) <= static_cast<int>(Stage::Hold);
    }
    ...
    /// stopped / attack / hold stage, and high in all other stages.
    ...
    Event process(float deltaTime) {
        ...
        case Stage::Attack:
            ...
            break;
        case Stage::Hold:
            // hold the max value of 1 for the hold time
            value = 1.f;
            time = hold;
            break;
        case Stage::Decay:
