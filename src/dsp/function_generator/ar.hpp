// An Attack-Release (AR) envelope generator.
...
#ifndef ENVELOPE_AR_HPP
#define ENVELOPE_AR_HPP
...
/// @brief An Attack-Release (AR) envelope generator.
struct AR {
 public:
    /// the different stages in the envelope.
    enum class Stage { Stopped = 0, Attack, Sustain, Release, NumStages };

    /// the events that output from the envelope generator at the end of stages
    enum class Event { None = 0, EndOfAttack, EndOfSustain, EndOfRelease };
 ...
 public:
    ...
    /// @brief Return the current stage of the AR.
    inline Stage getStage() const { return stage; }

    /// @brief Return true if the current stage is stopped.
    inline bool isStopped() { return stage == Stage::Stopped; }

    /// @brief Return true if the current stage is attack.
    inline bool isAttack() { return stage == Stage::Attack; }

    /// @brief Return true if the current stage is sustain.
    inline bool isSustain() { return stage == Stage::Sustain; }

    /// @brief Return true if the current stage is release.
    inline bool isRelease() { return stage == Stage::Release; }
